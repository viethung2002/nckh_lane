...
def parse_args():
    parser = argparse.ArgumentParser(description="Training Lane Detection Models (LaneNet/CSNN)")

    # General arguments
    parser.add_argument("--dataset", required=True, help="Dataset path, which should contain train.txt and val.txt")
    parser.add_argument("--model_type", choices=['lanenet', 'csnn'], help="Model type to train: lanenet or csnn", default='lanenet')
